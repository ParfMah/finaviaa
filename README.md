...
## 10. Images à remplacer

Conformément au cahier des charges, les visuels professionnels n'étant pas fournis, des emplacements `IMAGE À REMPLACER` ont été créés (classe `.img-placeholder`) à chaque endroit stratégique (hero, services, équipe, etc.). Recherchez `IMAGE À REMPLACER` dans les fichiers HTML pour les localiser rapidement. Remplacez-les par de vraies images au format **WebP**, avec des attributs `alt` déjà présents ou à compléter pour le SEO.
